...
- Always define specific TypeScript types for data you own: component props, Redux state, service return values, API response shapes.
- Prefer Zod-inferred types (`z.infer<typeof Schema>`) over manually written interfaces for validated data — they stay in sync automatically.
- Use `unknown` (never `any`) at external data boundaries — the exact point where third-party or API data enters the app before transformation and validation.
- Once data has passed through Zod validation, it must be fully typed everywhere downstream — no `any`, no `as unknown`.
- Always add explicit return types to non-trivial functions so TypeScript can catch shape mismatches early.
...
- All secrets come from environment variables in `.env`.
...
- Define all required variables in `.env`.
